...
func (repository *ElasticRepository) FilterByIndex(index string) ([]string, error, int) {
	...
	if err != nil {
		return logs, getError(err), http.StatusInternalServerError
	}
	...
	if err != nil {
		...
		return logs, err, http.StatusInternalServerError
	}

	if _, ok := result["hits"]; !ok {
		fmt.Println("An error occurred while fetching logs..Result obtained is null", result)
		err := errors.New("An Error Occurred..Please Enter a valid Index")
		return nil, err, http.StatusBadRequest
	}

	logs = getRelevantLogs(result)

	return logs, nil, http.StatusOK

}
func (repository *ElasticRepository) FilterByTime(startTime time.Time, finishTime time.Time) ([]string, error, int) {
	...
	if _, ok := result["hits"]; !ok {
		fmt.Println("An error occurred while fetching logs..Result obtained is null", result)
		err := errors.New("An Error Occurred..Please Enter time in the correct format")
		return nil, err, http.StatusBadRequest
	}
...
func (repository *ElasticRepository) FilterByPodName(podName string) ([]string, error, int) {
	...
	if _, ok := result["hits"]; !ok {
		fmt.Println("An error occurred while fetching logs..Result obtained is null", result)
		err := errors.New("An Error Occurred..Please Enter a valid Pod Name")
		return logs, err, http.StatusBadRequest
	}
...
func (repository *ElasticRepository) GetAllLogs() ([]string, error, int) {
	esClient := repository.esClient
	searchResult, err := esClient.Search(
		esClient.Search.WithContext(context.Background()),
		esClient.Search.WithIndex(constants.InfraIndexName, constants.AuditIndexName, constants.AppIndexName),
